...
% All CANONICAL nonequivalent permutated partitions of a given integer

function result = int_part_perm_canon( n )

datafile = sprintf( 'int_part/int_part_perm_canon_%i.mat', n );
...
for j = 1 : length( result0 )
    ...
    P_uniq = zeros( size(P) );
    for k = 1 : size( P, 1 )
        P_uniq(k,:) = pathcode_canon( P( k, : ) );
    end
    P_uniq = unique( P_uniq, 'rows' );
    for k = 1 : size( P_uniq, 1 )
        result{ end+1, 1 } = P_uniq( k, : );
    end
end

% Sort result by length:
len_result = zeros( 1, length( result ) );
for j = 1 : length( result )
    len_result(j) = length( result{j} );
end
[ ~, ix_sort ] = sort( len_result, 'descend' );
result0 = result;
for j = 1 : length( result )
    result{j} = result0{ ix_sort(j) };
end
